...
import { VBtn, VChip, VRow, VCol, VCard, VCardItem, VCardActions } from 'vuetify/components';
                    ...
                    <div class="d-flex justify-between">
                        <div class="d-flex flex-column justify-around">
                            <v-chip class="text-caption my-2 mr-2 item-justify-start">
                                <a :href="state.redirects?.filter(redirect => shortlink.short_code === redirect.short_code)[0]?.redirect" target="_blank">
                                    {{ state.redirects?.filter(redirect => shortlink.short_code === redirect.short_code)[0]?.redirect }}
                                </a>
                            </v-chip>
